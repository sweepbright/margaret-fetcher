...
describe('parseJson', () => {
    ...
    it('does not crash on 204 responses', () => {
        return fetch('http://google.com/empty')
            .then(parseJson)
            .then(response => {
    ...
    it('ignores empty responses', () => {
        return fetch('http://google.com/created')
            .then(parseJson)
            .then(response => {
                expect(response.data).toBeFalsy();
            });
    ...
    it('can parse contents of error responses', () => {
        return fetch('http://google.com/error')
            .then(parseJson)
            .catch(response => {
    ...
    it('can parse html error respnses', () => {
        return fetch('http://google.com/html-error')
            .then(parseJson)
            .catch(response => {
                expect(response.message).toEqual('Unprocessable Entity');
